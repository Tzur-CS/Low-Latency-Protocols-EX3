 ...
 */
void map_set(map m, const char *key, void *value, int value_len) {
    int b = hash(key) % m->capacity;
...
//const char *map_remove(map m, const char *key) {
    int b = hash(key) % m->capacity;
...
void print_map(map m) {
    const char *curr_key = map_first(m);
    printf("cur_key : %s\n", curr_key);

    printf("key: ( %s - %p ) , value: ( %s - %p )\n",
           curr_key, curr_key,
           (char *) ((struct Cell *) (map_get(m, curr_key)))->value,
           (char *) ((struct Cell *) (map_get(m, curr_key)))->value);
    curr_key = map_next(m, curr_key);
    while (curr_key != NULL) {
        printf("key: ( %s - %p ) , value: ( %s - %p )\n",
               curr_key, curr_key,
               (char *) ((struct Cell *) (map_get(m, curr_key)))->value,
